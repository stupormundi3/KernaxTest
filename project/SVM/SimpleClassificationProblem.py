...
from legacy.Kernax.LinearKernel import LinearKernel
...
for i in range(nSamples):
  ...
  print(a)
  print(f"A is above, iteration{c}"  )
  print(b)
  print(f"B is above, iteration {c}")
  c = c+1
  X[i][0], X[i][1] = a, b
...
# SVM train.
clf = svm.SVC(kernel = jit(my_kernel))
clf.fit(X, y)
...
Xr = jnp.arange(x_min, x_max, .02)
Yr = jnp.arange(y_min, y_max, .02)
Z = jnp.zeros(Z.shape)
for i in range(Xr.shape[0]):
  x = Xr[i]
  for j in range(Yr.shape[0]):
    y = Yr[j]
    for z in jnp.arange(0, 2, .02):
      if jit(onBoundary(x, y, z, X3d, coef, sup, b)):
        Z[i, j] = z
        break
